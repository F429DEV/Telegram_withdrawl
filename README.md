...
四种玩法都能叠加邀请加成：**每把 1 个人拉进群，中奖权重就 +N**，倍数在创建面板的
「👥 邀请加成」里调（0 / 1 / 2 / 3 / 5 / 10 / 20）。

比如设成 +3：什么都不做的人权重是 1，拉了 4 个人的权重是 `1 + 4×3 = 13`，
中奖概率就是前者的 13 倍。积分玩法下还会和发言分叠加（发 10 条 + 拉 2 人 = `10 + 2×3 = 16`）。

怎么算「邀请」：用群里那条「某某添加了某某」的系统消息认定，谁拉的算谁的。几个边界：

- **只算本场抽奖开始之后**拉进来的人，开抽奖之前的老账不翻
- 同一个人在同一个群**只算一次**，踢出去再拉回来不会重复加权重
- 自己点邀请链接进群的，算他自己进的，不算任何人邀请
- 拉机器人进群不算

⚠️ 这种归因方式**只对手动拉人有效**。别人自己搜群进来、或者通过你分享的邀请链接进来，
系统消息里没有「谁拉的」这个信息，算不到任何人头上。想精确归因到每个邀请链接，
得让机器人给每个人发专属邀请链接，那是另一套做法。

另外，倍数调得高又不设门槛的话，拉小号刷权重是很划算的。真要防，
...
| `/list` | 本群进行中的抽奖和编号 |
| `/view [编号]` | 看某场抽奖的全部参数：玩法、名额、开奖条件、门槛、邀请加成、参与人数、权重前 5、中奖者。只有一场在进行时可以省略编号 |
| `/end [编号]` | 立即开奖（只有一个进行中时可以省略编号） |
| `/cancel [编号]` | 取消抽奖 |
| `/reroll 编号` | 重抽，排除上一轮中奖者 |
| `/verify 编号` | 看这场抽奖的完整排名（第 1 名到最后，前 20 名） |
...
种子只存在数据库里，不发到群里。`/verify` 给出的是排名列表（谁第 1、谁第 2），
不显示种子和分数。
