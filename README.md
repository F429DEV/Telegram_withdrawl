...
|---|---|---|
| 发送消息 | 发抽奖卡片和开奖结果 | 必须 |
| 删除消息 | 清理用户回复的临时消息（比如配置时输入的奖品名） | 建议 |
...
机器人在群里发的消息，默认 **30 秒后自动删掉**。下面三种例外，永远留着：

- **抽奖卡片** —— 带参与按钮那条，删了就没法报名了
- **开奖结果** —— 到点开奖、`/end`、`/reroll` 的中奖公布
- **创建面板** —— 管理员还在点按钮设置的那个

其余的（`/help`、`/view`、`/verify`、`/list`、`/viewmyinfo`、报名提示、各种报错）
都是看完即走的，30 秒后自己清掉。**私聊里的消息一律不删** ——
`/invite` 的链接、`/viewmyinfo` 的记录私信过去是要留着用的。

改秒数或者关掉，在 `.env` 里：

```
EPHEMERAL_SECONDS=30     # 0 = 完全不删
```

> 机器人删自己发的消息不需要管理员权限，所以这个功能不依赖群里给不给「删除消息」。
> 另外 30 秒对「回复这条消息输入奖品名」那种提示略紧，嫌短就把秒数调大。
